...
fn yaml_to_scalar(v: &MarkedYaml) -> Option<Scalar> {
    match &v.data {
        YamlData::Value(s) => match s {
            saphyr::Scalar::Boolean(b) => Some(Scalar::Bool(*b)),
            saphyr::Scalar::Integer(i) => Some(Scalar::Number(*i as f64)),
            saphyr::Scalar::FloatingPoint(f) => Some(Scalar::Number(f64::from(*f))),
            saphyr::Scalar::String(s) => Some(Scalar::String(s.to_string())),
            _ => None,
        },
        _ => None,
    }
